...
export default function Review() {
  ...
  const [cardShownTimestamp, setCardShownTimestamp] = useState<number>(
    Date.now(),
  );
  ...
  useEffect(() => {
    if (deckId) {
      db.decks
        .get(deckId)
        .then((d) => setDeckName(d?.name ?? null))
        .catch(() => {});
    }
  }, [deckId]);
    ...
    const messages = [
      {
        emoji: "🎉",
        title: "All done!",
        subtitle: "You crushed it! Take a well-deserved break.",
      },
      {
        emoji: "🏆",
        title: "Session complete!",
        subtitle: "Your brain just got a little stronger.",
      },
      {
        emoji: "🚀",
        title: "Nailed it!",
        subtitle: "Consistency is the key — see you next time!",
      },
      {
        emoji: "⭐",
        title: "Great work!",
        subtitle: "Every review brings you closer to fluency.",
      },
    ];
    ...
    <div className="p-4 pb-24 relative overflow-hidden">
      {isOffline && (
        <div className="mb-4 relative z-10">
          <OfflineBanner message="You're offline. Reviews will sync when you reconnect." />
        </div>
      )}
