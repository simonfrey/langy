...
function MascotFace({ x, y, w, color = BLUE }: { x: number; y: number; w: number; color?: string }) {
  const s = w / 96;
  const h = 62 * s;
  const rx = 12 * s;
  const eyeR = 3 * s;
...
export function Mascot({ className = '', size = 140 }: { className?: string; size?: number }) {
  const w = size * 0.69;
  const h = w * (62 / 96);
  const x = (size - w) / 2;
  const y = (size - h) / 2;
  ...
  return (
    <svg className={className} width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none" aria-hidden>
      <rect x={x - glowPad} y={y - glowPad} width={w + glowPad * 2} height={h + glowPad * 2} rx={18} fill={BLUE_GLOW} />
      <MascotFace x={x} y={y} w={w} />
      <circle cx={size * 0.04} cy={size * 0.09} r={size * 0.035} fill={BLUE_MUTED} opacity="0.5" />
      <circle cx={size * 0.96} cy={size * 0.06} r={size * 0.028} fill={BLUE_PALE} opacity="0.6" />
      <circle cx={size * 0.01} cy={size * 0.91} r={size * 0.028} fill={BLUE_DEEP} opacity="0.4" />
      <circle cx={size * 0.97} cy={size * 0.94} r={size * 0.035} fill={BLUE_MUTED} opacity="0.4" />
    </svg>
  );
}
...
export function BlobBackground({ className = '' }: { className?: string }) {
  const blobs = useMemo(() => {
    ...
    interface Blob {
      ...
      rx: number; ry: number; rotation: number; opacity: number; color: string;
  ...
  return (
    <div className={`pointer-events-none fixed inset-0 min-h-screen overflow-hidden ${className}`} aria-hidden>
      {blobs.map((b, i) => (
        <svg
          key={i}
...
export function CardStackIllustration({ className = '' }: { className?: string }) {
  return (
    <svg className={className} width="160" height="150" viewBox="0 0 160 150" fill="none" aria-hidden>
      <rect x="20" y="10" width="120" height="100" rx="30" fill={BLUE_GLOW} opacity="0.4" />
      <rect x="32" y="22" width="96" height="62" rx="12" fill={BLUE_LIGHT} transform="rotate(-4 80 53)" />
      <rect x="32" y="22" width="96" height="62" rx="12" fill="#8FBDD4" transform="rotate(2 80 53)" />
      <MascotFace x={32} y={22} w={96} />
      <rect x="63" y="104" width="34" height="28" rx="8" fill={BLUE_MUTED} />
      <line x1="80" y1="111" x2="80" y2="125" stroke="white" strokeWidth="2.5" strokeLinecap="round" />
      <line x1="73" y1="118" x2="87" y2="118" stroke="white" strokeWidth="2.5" strokeLinecap="round" />
...
export function SparkleIllustration({ className = '' }: { className?: string }) {
  ...
  return (
    <svg className={className} viewBox="0 0 100 100" fill="none" aria-hidden>
      <rect x={fx - 6} y={fy - 6} width={fw + 12} height={fw * (62 / 96) + 12} rx="16" fill={BLUE_GLOW} opacity="0.5" />
      <MascotFace x={fx} y={fy} w={fw} />
      <line x1="50" y1="16" x2="50" y2="10" stroke={BLUE_PALE} strokeWidth="3" strokeLinecap="round" />
      <line x1="76" y1="26" x2="81" y2="21" stroke={BLUE_PALE} strokeWidth="3" strokeLinecap="round" />
      <line x1="24" y1="26" x2="19" y2="21" stroke={BLUE_PALE} strokeWidth="3" strokeLinecap="round" />
      <line x1="84" y1="48" x2="90" y2="48" stroke={BLUE_PALE} strokeWidth="3" strokeLinecap="round" />
      <line x1="16" y1="48" x2="10" y2="48" stroke={BLUE_PALE} strokeWidth="3" strokeLinecap="round" />
...
export function CelebrationIllustration({ className = '' }: { className?: string }) {
  ...
  return (
    <svg className={className} width="180" height="160" viewBox="0 0 180 160" fill="none" aria-hidden>
      <rect x={fx - 18} y={fy - 18} width={fw + 36} height={62 + 36} rx="30" fill={BLUE_PALE} opacity="0.2" />
      <rect x={fx - 8} y={fy - 8} width={fw + 16} height={62 + 16} rx="22" fill={BLUE_PALE} opacity="0.2" />
      <MascotFace x={fx} y={fy} w={fw} />
      ...
      <rect x="15" y="80" width="10" height="10" rx="3" fill={BLUE_PALE} opacity="0.4" transform="rotate(15 20 85)" />
      <rect x="155" y="85" width="8" height="8" rx="2.5" fill={BLUE_DEEP} opacity="0.4" transform="rotate(-20 159 89)" />
    </svg>
  );
}
