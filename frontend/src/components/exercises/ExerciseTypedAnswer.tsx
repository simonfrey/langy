...
import { InstructionText, SourceSentenceBox, HintBox, PromptBox } from './shared';
...
export default function ExerciseTypedAnswer({ exercise, answer, setAnswer, gradeResult, onSubmit }: ExerciseComponentProps) {
  const variant = exercise.type.includes('error') ? 'error' as const : 'default' as const;

  return (
    <>
      <InstructionText text={exercise.instruction} />
      <SourceSentenceBox text={exercise.source_sentence} />
      {exercise.prompt && <PromptBox text={exercise.prompt} variant={variant} />}
