...
export default function ExerciseMultipleChoice({ exercise, answer, setAnswer, gradeResult, onSubmit }: ExerciseComponentProps) {
  return (
    <>
      ...
      {exercise.data?.image_url && (
        <div className="flex justify-center my-3">
          <img src={exercise.data.image_url} alt="Exercise" className="max-h-48 rounded-xl" />
        </div>
      )}
      ...
      {!gradeResult && (
        <div className="grid grid-cols-2 gap-2 mt-3">
          {(exercise.options || []).map((option, i) => (
            <button
              key={i}
              onClick={() => { setAnswer(option); requestAnimationFrame(() => onSubmit()); }}
