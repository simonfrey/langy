          ...
          {i < parts.length - 1 && (
            gradeResult ? (
              ...
              <input
                ...
                style={{ width: `${Math.max(correctAnswer.length / blankCount, 4) * 0.65}em` }}
              />
            )
          )}
        </span>
...
export function HintBox({ hint, hideIfSourceSentence, sourceSentence }: { hint?: string; hideIfSourceSentence?: boolean; sourceSentence?: string }) {
...
export function PromptBox({ text, variant }: { text: string; variant?: 'error' | 'default' }) {
...
export function exerciseNeedsBlanks(type: string): boolean {
  const TYPES_WITHOUT_BLANKS = new Set([
    ...
    'grammar_error_correction', 'grammar_transformation', 'grammar_conjugation_drill',
    'vocab_matching_pairs', 'vocab_picture_word', 'grammar_categorization', 'grammar_matching',
    'integrative_reading',
  ]);
...
export function exerciseUsesCustomUI(type: string): boolean {
  const CUSTOM_UI_TYPES = new Set([
    'vocab_matching_pairs', 'vocab_word_bank', 'vocab_picture_word',
    'grammar_reorder', 'grammar_multiple_choice',
    'grammar_categorization', 'grammar_matching',
    'integrative_reading',
    'word_order_scramble',
  ]);
