...
export default function ExerciseMatchingPairs({ exercise, setAnswer, gradeResult }: ExerciseComponentProps) {
  ...
  const leftItems = pairs.map(p => isGrammar ? (p.left || '') : (p.native || ''));
  const rightItems = pairs.map(p => isGrammar ? (p.right || '') : (p.target || ''));

  // Shuffle right side, reset when exercise changes
  const [shuffledRight, setShuffledRight] = useState<{ item: string; origIdx: number }[]>([]);
  ...
  return (
    <>
        ...
        <div className="space-y-2">
          {leftItems.map((item, i) => {
            ...
            return (
              <button
                key={i}
                onClick={() => !isMatched && !gradeResult && handleTap('left', item, i)}
        ...
        <div className="space-y-2">
          {shuffledRight.map(({ item }, i) => {
            ...
            return (
              <button
                key={i}
                onClick={() => !isMatched && !gradeResult && handleTap('right', item, i)}
                disabled={isMatched || !!gradeResult}
                className={`w-full px-3 py-3 rounded-xl border-2 font-medium transition text-center ${
                  isMatched ? 'bg-emerald-50 border-emerald-300 text-emerald-700 opacity-60' :
                  isWrong ? 'bg-red-50 border-red-300 text-red-700' :
      ...
      {allMatched && !gradeResult && (
        <p className="text-center text-emerald-600 font-bold mt-3">All pairs matched!</p>
      )}
    </>
  );
}
